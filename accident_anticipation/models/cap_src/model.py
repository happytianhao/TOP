...
from .bert import FuseModel, TextModel, opt
...
class Text(nn.Module):
    def __init__(self):
        ...
        # Load tokenizer once during initialization
        import os
        current_dir = os.path.dirname(os.path.abspath(__file__))
        abl_path = os.path.join(current_dir, 'bert') + os.sep
        self.tokenizer = BertTokenizer.from_pretrained(abl_path + 'modeling_bert/bert-base-uncased-vocab.txt')
...
class accident(nn.Module):
    def __init__(self,h_dim=256,n_layers=1,depth=4,adim=512,heads=8,num_tokens=49,c_dim=512,s_dim1=120,s_dim2=512,keral=[3,4],num_class=2):
    ...
    def forward(self,x,y):
        #x:rgb、y:label(positive,negative)
        losses = {'total_loss': 0}
        all_output=[]
        x_11 = x
        ...
        for i in range(x.size(1)):
            x1 =x_11[:, i]
            tokens_tensor, input_masks_tensors=self.text([""] * len(y))
            x= self.fusion(tokens_tensor,input_masks_tensors,x1)
            x = self.features(x)
            x=x.permute(0,2,1).contiguous()
            output1, hh = self.gru_net(x, hh)  # Update hidden state across time steps
            #also can output foucs_p
            foucs_p=self.deconv(x)
            L1 = self._exp_loss(output1, y,i, x.size(1), fps=30.0)
            loss_sum=L1.mean()
    ...
    def _exp_loss(self, pred, target, time, toa, fps=30.0):
            ...
            # Convert toa to tensor if it's an integer
            if isinstance(toa, int):
                toa = torch.tensor(toa, device=pred.device, dtype=pred.dtype)
